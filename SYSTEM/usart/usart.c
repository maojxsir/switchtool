...
u8 USART_RX_BUF[USART_REC_LEN];// ="relay_k3 1000 500\r\n";;     //接收缓冲,最大USART_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	...
	if(Res == 0x03)
	{
		g_uExitPwmFlag = 1;//recv CTRL+C(03),set exit flag 
		printf("\r\n>");
		return;
	}
	...
	if((USART_RX_STA&0x8000)==0)//接收未完成
	{
		if(USART_RX_STA&0x4000)//接收到了0x0d
		{
			...
			else 
			{
				...
				USART_RX_BUF[USART_RX_STA&0X3FFF]=Res ; //recv \n
				printf("\r\n>");
			}
		}
		else //还没收到0X0D
